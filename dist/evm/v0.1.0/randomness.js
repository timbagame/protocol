import { encodeAbiParameters, keccak256, parseAbiParameters, sha256, size, } from "viem";
import { evmDomain } from "./client.js";
export const EVM_RANDOMNESS_VERSION = "keccak256-domain-block-u256-v1";
export function commitmentFor(secret) {
    if (!/^0x[0-9a-fA-F]{64}$/.test(secret))
        throw new Error("Secret must be 32 bytes");
    return sha256(secret);
}
export function selectEvmWinnerFromEntropy(initialEntropy, participants) {
    if (!/^0x[0-9a-fA-F]{64}$/.test(initialEntropy) ||
    ...
    for (let round = 0; round < 32; round++) {
        ...
        entropy = keccak256(encodeAbiParameters(parseAbiParameters("bytes32,uint256"), [
            entropy,
            BigInt(round),
        ]));
    }
...
export function calculateEvmWinner(deployment, gameId, secret, commitment, lastEntryBlock, participants) {
    ...
    const entropy = keccak256(encodeAbiParameters(parseAbiParameters("uint256,address,bytes32,bytes32,uint256"), [
